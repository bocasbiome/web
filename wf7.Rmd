...
library(tidyverse) #JJS# Added
library(phyloseq)
library(microbiome)
library(ggpubr)
library(labdsv) #for indicator analysis
require(gdata) #JJS# Added
library(scales) #JJS# Added
library(patchwork) #JJS# Added
library(DT) #JJS# Added
library(cowplot)
library(plyr)
library(data.table)
...
It is also possible to estimate the variation of OOB error for each prevalence interval filtering. This is done by running the random forests classification for `n` times, determined by the user. The function will return a boxplot figure and a table for each classification error.
...
Finally, we can create a  stacked barchart for the PIME obtained.
...
```{r, eval=FALSE, echo=FALSE}
tiff("figures/p7/Barchart_PIME_65_ASV.tiff", height=15, width=26,
